...
class OrdersModuleController {
    ...
    def newOrder(){
        ...
        def chefAvailable = scheduleService.isAnyChefAvailable()
        if (!chefAvailable) {
            Log.logger(Log.ERROR, logId, "Crear nueva orden.", "No se puede crear una orden fuera del horario laboral del chef.", "data: $data")
            return respond(TypeError.resourceNotAvaliable("horario del chef", logId, response))
        }
        if (!orderTime) {
            Log.logger(Log.ERROR, logId, "Crear nueva orden.", "No se recibe el horario.", "data: $data")
            return respond(TypeError.missingParameter("Horario de pedido", logId, response))
        }
        try {
