...
class OrderModuleService {
    ...
    Time now = Time.valueOf(LocalTime.now(java.time.ZoneId.of("America/Mexico_City")))
    ...
    def listOrdersByUser(data, userId) {
        try {
            ...
            return [resp: [success: true, message: 'Ordenes listadas', orders: formattedOrders,],status: 200]
        }
        catch (e) {
            return [
            ...
            for (order in data) {
                ...
                if(dish.availableDishes != -1){
                    ...
                    if( order.quantityDish > dish.availableDishes){
                        return [resp: [success:false, message: "No hay suficnetes platillos para esta orden, solo quedan " + dish.availableDishes],status:404]
                    }
                    else if( order.quantityDish == 0 ){
                        return [resp: [success:false, message: "Se a agotado este platillo"],status:404]
                    }
    ...
    def editOrder(dataP, dataR) {
        try {
            ...
            if (dataP.uuidItem){
                ...
                if (!orderItem) {
                    return [resp: [success: false, message: 'Platillo de la orden no encontrado'], status: 400]
                }
                if (order.status in ["Finished", "Cancelled", "Preparing"]) {
                    return [resp: [success: false, message: "No se puede editar esta orden si ya esta " + order.status], status: 400]
                }
                def newDishObject = Dish.findByUuid(dataR.uuidDish)
                if (!newDishObject) {
                    return [resp: [success: false, message: 'El platillo no fue encontrado'], status: 400]
                }
                ...
                return [resp: [success: true, message: "Platillo editado con exito", orderItem:orderItem],status: 200]
            }
            def dish = Dish.findByUuid(dataR.uuidDish)
            def orderItems = new OrderItem([
                unitPrice: dish.cost,
            ...
            return [resp: [success: success, message: "Se ha actualizado tu orden", order: mapOrder(order)], status: 200]            
        } catch (e) {
