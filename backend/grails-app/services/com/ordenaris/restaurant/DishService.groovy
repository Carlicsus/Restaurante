...
class DishService {
    ...
    private String getConfiguredDishUploadDir() {
        return Holders.config?.getProperty('restaurant.images.upload-dir', String, null) ?: 
            Holders.config?.getProperty('restaurant.images.uploadDir', String, null) ?: 
            Holders.config?.restaurant?.images?.'upload-dir' ?: 
            Holders.config?.restaurant?.images?.uploadDir
    }
    ...
    Map saveDishImageByUuid(String uuid, MultipartFile file) {
        ...
        try {
            ...
            if (!dish) {
                ...
                return [resp: [success: false, message: "Datos inválidos para platillo"], status: 404]
            }
            saveDishImage(dish, file)
            return [resp: [success: true, message: "Imagen guardada exitosamente"], status: 200]
        } catch (e) {
            Log.logger(Log.ERROR, logId, "Guardar imagen platillo", "Error al guardar imagen", e.getMessage())
            return [resp: [success: false, message:  "Ha ocurrido un error al guardar la imagen."], status: 400]
        }
    }
    ...
    Map reloadDishImageByUuid(String uuid, MultipartFile file) {
        ...
        try {
            ...
            return [resp: [success: true, message: "Imagen actualizada"], status: 200]
        } catch (e) {
            Log.logger(Log.ERROR, logId, "Recargar imagen platillo", "Error al recargar imagen", e.getMessage())
            return [resp: [success: false, message: "Ha ocurrido un error al recargar la imagen."], status: 400]
        }
    }
    ...
    private List buildPaginatedDishes(Map params) {
    ...
    private def validateDishNotExists(String name, MenuType menuTypeObj, String logId, def dishData) {
        def existingDish = Dish.findByNameAndMenuType(name, menuTypeObj)
        if (existingDish) {
            Log.logger(Log.WARN, logId, "Validación fallida", "Intento de crear platillo duplicado", "params: ${dishData}")
            return [success: false, message: 'Ya existe un platillo con ese nombre para este tipo de menú']
        }
        return [success: true]
    }

    private Dish createAndSaveDish(def dishData, MenuType menuTypeObj) {
        def dishStatus = (dishData.availableDate != null) ? EntityStatus.INACTIVE : EntityStatus.ACTIVE
        new Dish([
            name: dishData.name,
            menuType: menuTypeObj,
            availableDate: dishData.availableDate != null ? dishData.availableDate : null,
            cost: dishData.cost,
        ...
        try {
            ...
            def data = roots.findAll { isWithinSchedule(it) }.collect { type ->
                ...
                if (dishes) [uuid: type.uuid, name: type.name, startTime: type.startTime, endTime: type.endTime, dishes: dishes]
                    ...
                    def submenu = MenuType.findAllByStatusNotEqualsAndParentType(EntityStatus.DELETED, type)
            ...
            return [resp: [success: true, data: data, message: "Platillos activos"], status: 200]
        } catch (e) {
            Log.logger(Log.ERROR, logId, "Error en listActiveDishes", e.getMessage(), null)
            return [resp: [success: false, message: "Error interno del servidor."], status: 500]
        }
    }
    ...
    def listAllDishes() {
        ...
        try {
            ...
            [resp: [success: true, data: data, message: "Todos los platillos (no borrados)"], status: 200]
        } catch (e) {
            Log.logger(Log.ERROR, logId, "Error en listAllDishes", e.getMessage(), null)
            return [resp: [success: false, message: "Error interno del servidor."], status: 500]
        }
    }
...
def getTopDishesChart(Integer days = null, Integer limit = null) {
    ...
    try {
        ...
        Log.logger(Log.INFO, logId, "Resultado exitoso", "Top platillos más vendidos obtenido", "result: ${chartData.take(limit)}")
        return [
            resp: [
                success: true,
        ...
        Log.logger(Log.ERROR, logId, "Error en getTopDishesChart", e.getMessage(), "params: { days: ${days}, limit: ${limit} }")
...
def newDish(dishData) {
    ...
    try {
        ...
        if (menuTypeObj instanceof Map && menuTypeObj.typeError) {
            return [
                resp: [success: false, message: "No se puede crear porque el tipo de menú no existe"],
                status: 404
            ]
        }
        def vName = DishValidationUtils.validateDishName(dishData.name, grailsApplication)
        if (vName instanceof Map && vName.success == false) return [resp: vName, status: 400]
        def vMenuType = DishValidationUtils.validateMenuType(dishData.menuType)
        if (vMenuType instanceof Map && vMenuType.success == false) return [resp: vMenuType, status: 400]
        def vDesc = DishValidationUtils.validateDishDescription(dishData.description, grailsApplication)
        if (vDesc instanceof Map && vDesc.success == false) return [resp: vDesc, status: 400]
        def vAvail = DishValidationUtils.validateAvailableDishes(dishData.availableDishes, grailsApplication)
        if (vAvail instanceof Map && vAvail.success == false) return [resp: vAvail, status: 400]
        def vCost = DishValidationUtils.validateDishCost(dishData.cost)
        if (vCost instanceof Map && vCost.success == false) return [resp: vCost, status: 400]
        def vCostMax = DishValidationUtils.convertAndValidateCost(dishData.cost, grailsApplication)
        if (vCostMax instanceof Map && vCostMax.success == false) return [resp: vCostMax, status: 400]
        dishData.cost = vCostMax
        def vUnique = validateDishNotExists(dishData.name, menuTypeObj, logId, dishData)
        if (vUnique instanceof Map && vUnique.success == false) return [resp: vUnique, status: 409]
...
def incrementAvailableDishes(uuid, quantity) {
    ...
    try {
        ...
        if (dish instanceof Map && dish.status) {
            ...
            return [resp: [success: false, message: (msg ?: "Error de validación").toString()], status: dish.status]
        }
        ...
        if (e instanceof TypeError) {
            return [resp: [success: false, message: "Error interno del servidor."], status: 400]
        }
        return TypeError.internalError(logId)
    }
}
...
def dishInfo(uuid, requestedStatus = null) {
    ...
    if (!uuid || uuid.toString().trim().size() != 32) {
        ...
        return [resp: [success: false, message: "Access is denied"], status: 403]
    }
    def dishOrError = validateDishInfoParams(uuid, logId)
    if (dishOrError instanceof Map && dishOrError.status) {
        return [
            resp: dishOrError.data ?: [success: false, message: "Error desconocido"],
...
def editDish(dishData) {
    ...
    try {
        ...
        if (menuTypeResult && menuTypeResult.error) {
            return [
                resp: [success: false, message: menuTypeResult.message ?: "El tipo de menú no existe"],
                status: 400
            ]
        }
        ...
        return [
            resp: [success: true, message: "Platillo actualizado correctamente"],
            status: 200
        ]
    } catch (e) {
        ...
        return [resp: [success: false, message: msg], status: statusCode]
    }
}
def editDishStatus(status, uuid) {
    def logId = UUID.randomUUID().toString().replaceAll('-', '')
    Log.logger(Log.INFO, logId, "Editar status de platillo", "Inicia Solicitud", "params: { status: ${status}, uuid: ${uuid} }")
    def dish = Dish.findByUuid(uuid)
    if (!dish) {
    ...
    try {
        dish.status = status
        dish.save(flush: true, failOnError: true)
        Log.logger(Log.INFO, logId, "Resultado exitoso", "Estado del platillo actualizado", "result: ${dish.uuid}, status: ${status}")
        return [
            resp: [success: true, message: "Estado del platillo actualizado"],
            status: 200
        ]
    } catch (e) {
...
def paginateDishes(Map params) {
    ...
    def errorMsg = com.ordenaris.restaurant.DishValidationUtils.validatePaginateDishesParams(params, logId, grailsApplication)
    if (errorMsg) {
        return [resp: [success: false, message: errorMsg], status: 400]
    }
    try {
        def paginatedDishes = buildPaginatedDishes(params)
        Log.logger(Log.INFO, logId, "Resultado exitoso", "Platillos paginados", "result: ${paginatedDishes}")
        return [
            resp: [
                success: true,
                data: [message: "Platillos paginados", chartData: paginatedDishes]
            ],
            status: 200
        ]
    } catch (e) {
        Log.logger(Log.ERROR, logId, "Error en paginateDishes", e.getMessage(), "params: ${params}")
        return [
            resp: [success: false, message: "Ha ocurrido un error, intenta más tarde."],
            status: 500
        ]
    }
}
...
def getTopSellingDishes(int limit = 10) {
    ...
    private String resolveBasePath() {
        def configuredPath = Holders.config?.repository ?: Holders.config?.app?.upload?.basePath
        if (!configuredPath) {
    ...
    private Path resolveDishImagesDir() {
        def configuredUploadDir = getConfiguredDishUploadDir()
        if (configuredUploadDir) {
            return Paths.get(configuredUploadDir.toString())
        }
