                ...
                for (int i = 0; i < data.size(); i++){
                    ...
                    if(dish.availableDishes != -1){
                        ...
                        if(dish.availableDishes < d.quantityDish){
                            ...
                            return TypeError.resourceNotAvaliable("Los platillos no logran abastecer el carrito de compras", logId)
                        }
                    }
        ...
        try {
            ...
            if (data.status == "Finished") {
                ...
                if (!scheduleService.isAnyChefAvailable()) {
                    Log.logger(Log.ERROR, logId, "Editar estatus del carrito.", "La cocina se encuentra cerrada a esta hora.", "status: ${data.status}")
                    return TypeError.resourceNotAvaliable("la cocina esta cerrada.", logId)
                }
                def user = User.findById(shoppingCart.user.id) 
                if(data.status=="Finished" && !user.enabled){
                    ...
                    return TypeError.resourceNotAvaliable("que tu cuenta esta desactivada.", logId)
                }
                try {
                    ...
                    return TypeError.missingParameter("formato de hora invalido.", logId)
                }
                ...
                return TypeError.incorrectFormat("formato de hora invalido.", logId)
            }
        }
        catch (e) {
                    ...
                    if(item.dish.uuid == requestBody.dishUuid){
                        ...
                        if(dish.availableDishes != -1){
                            ...
                            if(item.dish.availableDishes < newQuantityDish){
                                Log.logger(Log.ERROR, logId, "Agregar nuevo platillo.", "SNo hay suficientes productos disponibles.", "json: $requestBody, params: $pathParams")
                                return TypeError.resourceNotAvaliable("Los platillos no logran abastecer el carrito de compras", logId)
                            }
                        }
