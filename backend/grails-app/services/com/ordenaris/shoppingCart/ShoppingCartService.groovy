...
class ShoppingCartService {
    ...
    def listOrderShoppingCart(data, logId) {
        try{
            Log.logger(Log.INFO, logId, "Consultar carrito de compras.", "Llega al servicio.", ":D")
    ...
    def newOrderShoppingCart(data, auth, logId) {
        try {
            ...
            if(shoppingCartItems == []){
                ...
                for (int i = 0; i < data.size(); i++){
                    ...
                    if(existingItem){
                            ...
                            newQuantityDish = existingItem.quantity + d.quantityDish
            ...
            Log.logger(Log.INFO, logId, "Crear carrito de compras.", "Se creo con exito el carrito de comrpas.", "data: $data, auth: ${auth.username}")
            return [resp: [success: true, message: "Orden agregada al carrito de compras"], status: 201]
        }
        catch (e) {
    ...
    def addNumberDish(requestBody, pathParams, logId){
        try{
            ...
            newQuantityDish = shoppingCartItem.quantity + requestBody.quantityDish
            if(dish.availableDishes != -1){
        ...
        try{
            ...
            if(incomingDishUuid in existingDishUuids){
                for (item in shoppingCartItem){
                    Log.logger(Log.INFO, logId, "Crear carrito de compras.", "Se verifica si el platillo de la orden y platillo enviado son iguales.", "json: $requestBody, params: $pathParams")
                    if(item.dish.uuid == requestBody.dishUuid){
                        Log.logger(Log.INFO, logId, "Crear carrito de compras.", "Son iguales, se agregara la cantidad del platillo.", "json: $requestBody, params: $pathParams")
                        ...
                        if(dish.availableDishes != -1){
                            Log.logger(Log.INFO, logId, "Crear carrito de compras.", "El platillo tiene una cantidad fija en la base de datos.", "json: $requestBody, params: $pathParams")
                            if(item.dish.availableDishes < newQuantityDish){
                                Log.logger(Log.INFO, logId, "Crear carrito de compras.", "Se verifica que haya suficientes productos para agregarlos al carrito de compras.", "json: $requestBody, params: $pathParams")
                                return [resp: [success: false, message: "No hay suficientes platillos para añadir al carrito."], status: 404]
                            }
                        }
                        Log.logger(Log.INFO, logId, "Crear carrito de compras.", "Se sumara la cantidad del producto al que esta en la base de datos.", "json: $requestBody, params: $pathParams")
                ...
                if(requestBody.quantityDish==null || requestBody.quantityDish < 1){
                    Log.logger(Log.INFO, logId, "Crear carrito de compras.", "Fin de la solicitud, ya no hay mas platillos para agregar.", "json: $requestBody, params: $pathParams")
                    return [resp: [success: true, message: "Orden agregada al carrito de compras"], status: 201]
                }
                dish = Dish.findByUuid(requestBody.dishUuid)
                if(dish.availableDishes != -1){
                ...
                Log.logger(Log.INFO, logId, "Crear carrito de compras.", "Se crean los productos en el carrito de compras.", "json: $requestBody, params: $pathParams")
                def shoppingCartItemEntry = new ShoppingCartItem([
                    userId: auth.id,
                    dish: dish.id,
    ...
    def deleteItemShoppingCart(data, logId) {
            try{
            Log.logger(Log.INFO, logId, "Agregar nuevo platillo.", "Llego al servicio.", "json: $data")
            def shoppingCart = ShoppingCart.findByUuid(data.uuidSC)
            if (!shoppingCart) {
                Log.logger(Log.INFO, logId, "Agregar nuevo platillo.", "No existe el carrito de compras.", "json: $data")
                return [resp: [success: false, message: "Carrito de compras no encontrado"], status: 404]
            }
            def cartItem = ShoppingCartItem.findByUuid(data.cartItem)
            if (!cartItem) {
                Log.logger(Log.INFO, logId, "Agregar nuevo platillo.", "No existe el platillo del carrito de compra.", "json: $data")
            ...
            Log.logger(Log.INFO, logId, "Agregar nuevo platillo.", "Se elimino el platillo con exito.", "json: $data")            
